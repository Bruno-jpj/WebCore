...
def handle_post_call(api_data: dict, request: HttpRequest):
    ...
    if client_key is not None:
        ...
        machine_category = api_data("machine_category") # ex: TR
        ...
        try:
            ...
            try:
                ...
                return{"status": status.HTTP_404_NOT_FOUND, "data": "Query DB non riuscita [1]"}
            #
        except Macchinari.DoesNotExist:
            machine_obj = Macchinari.objects.all()
            
            info_qs = Informazioni.objects.select_related('id_macchinario', 'id_allarme').filter(
                id_allarme__titolo=alarm_obj.titolo,
                id_macchinario__piano_produzione=machine_obj
                ).values(
                    'id',  # corrisponde all'id della tab. Informazioni
                    'id_macchinario__piano_produzione',
                    'id_allarme__titolo',
            ...
            try:
                ...
                return{"status": status.HTTP_404_NOT_FOUND, "data": "Query DB non riuscita [2]"}
            #
        except AllarmiSoluzioni.DoesNotExist:
            alarm_obj = AllarmiSoluzioni.objects.all()

            info_qs = Informazioni.objects.select_related('id_macchinario', 'id_allarme').filter(
                id_allarme__titolo=alarm_obj,
                id_macchinario__piano_produzione=machine_obj.piano_produzione
                ).values(
                    'id',  # corrisponde all'id della tab. Informazioni
                    'id_macchinario__piano_produzione',
            ...
            api_logger_view(e, "API try-except mentre ricerca obj nel DB")
        #
